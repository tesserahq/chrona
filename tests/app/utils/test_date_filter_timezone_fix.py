...
class TestDateFilterTimezoneFix:
    ...
    def test_digest_date_range_with_america_new_york_timezone(self):
        ...
        assert start_date_nyc == expected_start, (
            f"Start date should be Sep 24 midnight NYC, got {start_date_nyc}"
        )
        assert end_date_nyc == expected_end, (
    ...
    def test_digest_date_range_covers_entry_update_in_backfill_scenario(self):
        ...
        tz = pytz.timezone(timezone_str)
        sep25_execution = tz.localize(datetime(2025, 9, 25, 22, 0, 0))  # Sep 25, 10 PM NYC
        ...
        sep24_execution = tz.localize(datetime(2025, 9, 24, 22, 0, 0))  # Sep 24, 10 PM NYC
    ...
    def test_cron_timezone_bug_reproduction(self):
        ...
        prev_seconds_fixed = cron.previous(now=current_nyc_naive.timestamp(), default_utc=False)
        ...
        assert prev_execution_fixed == expected_execution, (
        ...
        prev_seconds_buggy = cron.previous(now=current_nyc.timestamp(), default_utc=True)
        ...
        time_difference = abs((prev_execution_fixed - prev_execution_buggy).total_seconds())
    ...
    def test_multiple_timezones_work_correctly(self):
        ...
        for timezone_str in test_timezones:
            ...
            expected_start_utc = (execution_time - timedelta(days=1)).replace(
